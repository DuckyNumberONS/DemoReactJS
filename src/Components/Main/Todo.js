import React from "react";

export default function Todo({ todos, completeTodo, removeTodo }) {

  return todos.map((todo, index) => (
    <ul
      className="mt-4"
      class ={todo.isComplete ? "todo-row complete" : "todo-row"}
      key={index}
    >
      <li
        className="flex items-center space-x-1 py-2.5 px-2.5 border-b border-gray-300 transition duration-300 ease-in no-underline text-gray-800"
        key={todo.id}
      >
        <input
          onClick={() => completeTodo(todo.id)}
          name="completed-checkbox"
          type="checkbox"
          className="form-checkbox rounded text-pink-600 shadow-none focus:shadow-none focus:ring-0 focus:ring-offset-0 focus:outline-none"
          value="false"
        ></input>
        <span className="flex-1 px-2 min-w-0 break-words">{todo.text}</span>
        <button onClick={() => removeTodo(todo.id)}>
      <svg
        stroke="currentColor"
        fill="currentColor"
        stroke-width="0"
        viewBox="0 0 448 512"
        height="1em"
        width="1em"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path d="M32 464a48 48 0 0 0 48 48h288a48 48 0 0 0 48-48V128H32zm272-256a16 16 0 0 1 32 0v224a16 16 0 0 1-32 0zm-96 0a16 16 0 0 1 32 0v224a16 16 0 0 1-32 0zm-96 0a16 16 0 0 1 32 0v224a16 16 0 0 1-32 0zM432 32H312l-9.4-18.7A24 24 0 0 0 281.1 0H166.8a23.72 23.72 0 0 0-21.4 13.3L136 32H16A16 16 0 0 0 0 48v32a16 16 0 0 0 16 16h416a16 16 0 0 0 16-16V48a16 16 0 0 0-16-16z"></path>
      </svg>
    </button>
      </li>
    </ul>
  ));
}
